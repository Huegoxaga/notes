...
- Music Alphabet `C D E F G A B` （then loop)
  - From the keyboard perspective: `C` is the left of the group of 2 black key.
  - From the frequency perspective: `C` is `A4` which has `440Hz`(variation applies)
...
- Grand staff
  ...
  - ![grand staff]()
- ledger line is the extra short line for notes outside the staff’s five lines.
...
- Minor scale has a minor tetrachord as the lower tetrachord.
- for melodic minor scale:
- the upper tetrachord is a major tetrachord.
- for harmonic minor scale:
- the upper tetrachord is `<HS><Mi3><HS>`
  - `<Mi3>` is a 3 half-steps interval.
- for natural minor scale:
  - the upper tetrachord is `<HS><WS><WS>`
...
- interval is described by a number which represents distance of these two notes on the staff.
- diatonic interval are those with an upper note belong to the major scale starts from the lower note.
- diatonic interval can be either perfect(upper notes is the 4th, 5th and 8th note of the current scale) or major(upper notes belongs to all others)
...
- The circle of fifths or the circle of fourths
  are tetrachords connected by `<WS>`s
...
- Sharps and flats add from left to right in the following order.
  - ![Sharp Key Signature]()
  - ![Flat Key Signature]()
- Triad
