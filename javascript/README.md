...
* Libraries
  * [jQuery](https://github.com/Huegoxaga/notes/tree/7206c75bae1427a1ae267d0529f2229f8f765b96/javascript/jquery.md)
  * [REACT](react.md)
* Framework
...
* `<script>` can have attribute type, , not required.
...
`let` declares local variables that only available in the current block of code. let keyword is recommended for variable declaration.
...
* async function: async function functionName\(\) { const msg = await scaryClown\(\); console.log\('Message:', msg\); }
* async function expressions: const functionName = async function\(\) {
...
* Content\_Type specifies the format that used to pass the parameters.
