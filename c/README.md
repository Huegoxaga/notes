...
- Declaration
  ...
  - `char *str = "stuff";` is the string pointer declaration, it is considered a constant and cannot be changed from its initial value.
...
- The `&` sign before the variable name is the address operator. It gives the address, or location in memory, of a variable.
...
- For a pointer of array x
  ...
  - _(x+1) or _(x++) is the second element and so on.
